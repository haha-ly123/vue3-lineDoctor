...
import { getMedicalOrderPre, getAddressList, createMedicalOrder } from '@/services/medicine';
...
// 获取药品数据--预订单
const getMedicalOrderPreApi = async () => {
  const { data } = await getMedicalOrderPre({ prescriptionId: route.query.id as string })
  orderPre.value = data
}
onMounted(() => {
  addressListApi()
...
const orderId = ref('')
// 通过预订单生成药品订单
const createMedicalOrderApi = async () => {
  if (!agree.value) return showToast('请勾选协议！')
  if (!address.value?.id) return showToast('请重新选择地址！')
  if (!orderPre.value?.id) return showToast('处方无效') 
  // 药品订单已存在
  if (orderId.value) {
    show.value = true
  } else {
...
<template>
  <div class="order-pay-page" v-if="orderPre && address">
    ...
    <van-submit-bar @click="createMedicalOrderApi" :loading="loading" :price="orderPre.actualPayment * 100"
      button-text="立即支付" button-type="primary" text-align="left"></van-submit-bar>
    <CpPaySheet v-model:show="show" :orderId="orderId" :actualPayment="orderPre.actualPayment + ''" :payCallback="'http://localhost:5173/order/pay/result'"></CpPaySheet>
  </div>
