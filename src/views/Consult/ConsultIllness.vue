...
const onAfterRead: UploaderAfterRead = (item) => {
    ...
    uploadImage(item.file)
        .then((res) => {
            ...
            form.value.pictures?.push(res.data)//fileList已经存在，此处是用于回显
            store.setIllness(form.value)
        })
        .catch(() => {
            item.status = 'failed'
...
onMounted(() => {
    if (store.consult.illnessDesc) {
        showConfirmDialog({
            ...
            closeOnPopstate: false,//返回时默认自动关闭弹窗，关闭默认
...
<template>
    <div class="consult-illness-page">
        ...
        <div class="illness-form">
            <van-field type="textarea" v-model="form.illnessDesc" rows="3" placeholder="请详细描述您的病情，病情描述不能为空"></van-field>
            <div class="item">
                <p>本次患病多久了？</p>
            ...
            <div class="illness-img">
                <van-uploader upload-icon="photo-o" :after-read="onAfterRead" @delete="onDeleteImg" v-model="fileList"
                    upload-text="上传图片" max-count="9" :max-size="5 * 1024 * 1024"></van-uploader>
                <p class="tip">上传内容仅医生可见,最多9张图,最大5MB</p>
            </div>
            <van-button type="primary" block round :class="{ disabled: disabled }" @click="nextFn">下一步</van-button>
        </div>
